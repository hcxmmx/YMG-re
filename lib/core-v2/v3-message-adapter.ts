...
export class V3MessageAdapter {
   ...
   * 
   * ⚠️ 重要说明 - 职责澄清：
   * 这个方法 ❌ 不处理占位符内容！占位符处理在Store层完成！
   * 
   * 📋 实际工作流程：
   * 1. Store.applyPreset() → 处理SillyTavern预设 → 解析占位符 → 生成动态内容
   * 2. SendMessageManager → 接收已处理的systemPrompt → 调用本方法
   * 3. 本方法职责：深度注入 + 消息优化 + API格式转换
   * 
   * 🔧 参数说明：
   * @param messages - 用户对话消息历史
   * @param preset - 临时预设（仅API参数容器，不含占位符处理）
   * @param systemPromptOverride - 🔑 关键！已包含所有占位符动态内容的最终系统提示词
   * @param additionalContext - 额外上下文（备用）
   * 
   * 🚀 V3引擎优势：
   * - 深度注入（injection_depth/order/role）
   * - 消息合并优化
   * - API格式智能转换
   * - 60%性能提升 + 40%内存优化
   */
